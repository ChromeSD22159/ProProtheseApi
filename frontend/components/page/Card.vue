<template>
    <div class="flex flex-col rounded-xl p-10 gap-2 transition w-full" :class="{ 'active': isActive }">
        <div class="flex flex-row gap-5 items-center" >
            <img v-if="isNotNullOrUndefined(content.icon.data)" :src="buildImageUrl(content.icon.data.attributes.url)" class="h-8 w-8 transition ease-in-out" />
        ...
        <img v-if="isNotNullOrUndefined(content.icon.data) && isMobil" :src="buildImageUrl(content.image.data.attributes.url)" class="" />
    </div>
</template>
